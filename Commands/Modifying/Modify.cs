...
{
	Logger.Info($"{property.Name}: {property.GetValue(instance)}");
}

properties[1].SetValue(instance, "#00FF00");
Vector3 position = mapEditorObject.RelativePosition + Vector3.up;
properties[3].SetValue(instance, position.ToString("G"));

foreach (MapEditorObject sus in MapUtils.LoadedMaps[mapEditorObject.MapName].SpawnedObjects)
{
	if (sus.Id != mapEditorObject.Id)
		continue;
